  ...
  try {
    ...
    res.status(200).json({
      ...
      totalPages: Math.ceil(
        (await User.countDocuments({ role: "writer" })) / pageSize
      ),
      totalWriters,
    });
  } catch (error) {
...
const getUser = async (req, res) => {
  ...
  try {
    const user = await User.findById(id).select(
      "-password -_id -otp -otpExpiry -otpSecret -encryptedTokenIv"
    );
...
export { deleteUser, getAllWriters, getUser, searchWriter, updateUser };
